...
//                            count++;
    ...
    @Override
//自動產生的第二個參數有 @IntDef(value = {Service.START_FLAG_REDELIVERY, Service.START_FLAG_RETRY}, flag = true)
    public int onStartCommand(Intent intent, int flags, int startId) {
//        t.start();//當希望被重複呼叫時,就放在onStartCommand裏啟動,但執行緒尚未結束前不可以產生另一個執行緒,要把run移下來
        Toast.makeText(this,"啟動服務",Toast.LENGTH_SHORT).show();
        return super.onStartCommand(intent, flags, startId);
    }

    @Override
    public void onDestroy() {
        Toast.makeText(this,"結束服務",Toast.LENGTH_SHORT).show();
